    ...
    describe('UC-301: Add meal /api/meal', () => {
        ...
        it('TC-301-1: When a required input is missing, a valid error should be returned', (done) => {
            chai.request(server)
                .post('/api/meal')
                .set({
                    Authorization: 'Bearer ' + jwt.sign({ userId: 1 }, jwtSecretKey),
                })
                .send({
                    // name is missing
                    ...
                    imageUrl: 'https://img3.restameta.com/950/288/4938944649502884.jpg',
                    allergenes: ['noten'],
    ...
    describe('UC-302: Update meal /api/meal/:mealId', () => {
        ...
        it('TC-302-3: When the user doesn not own the meal, a valid error should be returned', (done) => {
            chai.request(server)
                .put(`/api/meal/${validMealId}`)
                .set({
                    Authorization: 'Bearer ' + jwt.sign({ userId: 3 }, jwtSecretKey),
                })
                .send({
                    name: 'Broodje krokante kip',
                    description: 'Een lekker broodje krokante kip van Same Same met zoet-zure saus',
